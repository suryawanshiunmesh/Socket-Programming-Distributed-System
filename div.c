	...
	{              
		...
		//display the connection message and ask to for the input
    		...
    		token=strtok(NULL, sym);
      		num2=atof(token);
      		//printf("value of num1 and num2 %d %d:",num1,num2);
		if(num2==0)
		{
		...
		//check the condition for the divide by zero error
		if(num2!=0)
		{
			num2 = num1/num2;
		...
		{
			strcpy(buffer,"Divide by 0 error..!\n");
			}
	     	n=write(newsockfd,buffer,strlen(buffer));
	     	if(n<0) 
	     		error("Error..! Writing to socket..!\n");
